...
public class BlockStackFlower extends BlockStack implements IMetaName{
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
	{
        if(!worldIn.isRemote)
        {
    		...
    		if((meta % 4) < 3)
        	{
    			...
    			{
    				worldIn.setBlockState(pos, ModuleBlocks.STACK_DEAD.getDefaultState().withProperty(FACING, state.getValue(FACING)));
    			}
        	}
        }
	}
	...
	private boolean canDry(World world, BlockPos pos, IBlockState state)
	{
		...
		if (world.getBlockState(pos.offset(face.rotateY())).isOpaqueCube() || world.getBlockState(pos.offset(face.rotateYCCW())).isOpaqueCube()) {return false;}
    ...
    public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand)
    {
    	if (rand.nextInt(5) == 0 && getMetaFromState(stateIn) % 4 < 3)
    	{
	        ...
	        Vec3d position = new Vec3d(d0 + 0.38D * (double)enumfacing.getFrontOffsetX(), d1 - 0.22D, d2 + 0.38D * (double)enumfacing.getFrontOffsetZ());
